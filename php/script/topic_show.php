...
if(isset($_POST['page']) && isset($_POST['page_size']) && isset($_POST['data'])) {
	try {
	    ...
	    $query = $bdd->prepare('SELECT topic.id, title, topic.content, DATE_FORMAT(topic.date, "%d/%m/%y - %Hh%i") as date_, login, user.id as login_id, answered, pot_point, COUNT(post.id) as nb_post, topic.pot_point as points
								FROM topic
